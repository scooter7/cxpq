                                ...
                                if len(least_represented_images_q9) == 3:
                                    ...
                                    if st.button("Submit"):
                                        if len(selected_traits_q1) != 3:
                                            ...
                                            st.warning("Please select an image for Q8.")
                                            ...
                                            st.session_state.top_two_colors = top_two_colors
                                            st.session_state.persona_name = persona_name
                                            st.session_state.score_counter = score_counter
                                            st.session_state.submitted = True
                                            st.session_state.random_seed = random.randint(0, 1000)

    if st.session_state.submitted:
        st.write(f"Your top two colors are: {', '.join(st.session_state.top_two_colors)}")
        st.write(f"Your persona name is: {st.session_state.persona_name}")
        st.write(f"Your scores for each color are: {dict(st.session_state.score_counter)}")
